...
def get_cls_embedding_pretrained(model, image_tensor):
    with torch.no_grad():
        outputs = model(image_tensor)
    cls_embedding = outputs.last_hidden_state[:, 0, :]  # Το πρώτο token είναι το CLS token  
...
def calculate_average_distance(case_folder):
    ...
    cls_embeddings = [get_cls_embedding_pretrained(pretrained_model, load_and_transform_image(image_path)) for image_path in image_paths]
    
    print("1: Shape of cls_embeddings[0]:", cls_embeddings[0].shape)
    print("1: Type of cls_embeddings[0]:", type(cls_embeddings[0]))
...
for i in range(1, 3):
    ...
    pretrained_cls_distances.append((f"CASE{i:03d}", dist_01, dist_02, avg_distance))
...
# Προσθήκη/ενημέρωση στήλης pretrainedViTCLS
#df['pretrainedViTCLS'] = [dist[3] for dist in pretrained_cls_distances]
