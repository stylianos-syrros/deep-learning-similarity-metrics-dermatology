...
# Ορισμός του Logger
logging.basicConfig(filename=r'D:\Diploma\ISIC_log\training_from_epoch_log.log', level=logging.INFO,
...
best_model_path = os.path.join(model_dir, "best_vit_model.pth")
model = timm.create_model('vit_base_patch16_224', pretrained=True, num_classes=len(full_train_dataset.classes))
...
if os.path.exists(best_model_path):
    ...
    print(f"Loaded best model from epoch {checkpoint['epoch']} with validation loss {best_val_loss:.4f}.")
    logging.info(f"Loaded best model from epoch {checkpoint['epoch']} with validation loss {best_val_loss:.4f}.")
...
train_model(model, train_loader, val_loader, criterion, optimizer, start_epoch=start_epoch, num_epochs=25)
