...
# Ορισμός του Logger
logging.basicConfig(filename=r'D:\Diploma\VGG16_ISIC_LOG\VGG16_training_from_start_log.log', level=logging.INFO,
...
model.classifier[6] = nn.Linear(model.classifier[6].in_features, 3)  # Αλλαγή του τελευταίου fully connected layer για 3 κατηγορίες
model.to(device)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=25, start_epoch=1, best_val_loss=float('inf')):
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        }, os.path.join(model_dir, f'vgg16_model_epoch_{epoch}.pth'))
        ...
        torch.save({
            ...
            'optimizer_state_dict': optimizer.state_dict(),
            'best_val_loss': best_val_loss,
        }, os.path.join(model_dir, 'last_checkpoint.pth'))
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'optimizer_state_dict': optimizer.state_dict(),
                'best_val_loss': best_val_loss,
            }, os.path.join(model_dir, 'best_vgg16_model.pth'))
...
# Χειροκίνητος καθορισμός της αρχικής εποχής
start_epoch = 13

# Φόρτωση του checkpoint της τελευταίας εποχής (epoch 12)
checkpoint_path = os.path.join(model_dir, 'vgg16_model_epoch_12.pth')
if os.path.exists(checkpoint_path):
    checkpoint = torch.load(checkpoint_path)
    model.load_state_dict(checkpoint['model_state_dict'])
    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    best_val_loss = checkpoint['best_val_loss']
    print(f"Continuing training from epoch {start_epoch} with best validation loss {best_val_loss:.4f}")
    logging.info(f"Continuing training from epoch {start_epoch} with best validation loss {best_val_loss:.4f}")
else:
    print("Checkpoint for epoch 12 not found. Starting training from scratch.")
    logging.info("Checkpoint for epoch 12 not found. Starting training from scratch.")

# Εκτύπωση αρχικών συνθηκών
...
train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=25, start_epoch=start_epoch, best_val_loss=best_val_loss)

# Τελική αξιολόγηση στο test set
checkpoint = torch.load(os.path.join(model_dir, 'best_vgg16_model.pth'))
model.load_state_dict(checkpoint['model_state_dict'])
