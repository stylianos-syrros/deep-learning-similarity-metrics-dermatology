...
# Προσαρμοσμένος μετασχηματισμός εισόδου
def custom_preprocess_input(img_array):
    # Δικές σας τιμές mean και std (cases)
    mean = np.array([0.65886277, 0.45850426, 0.41027424])
    std = np.array([0.14796351, 0.14156736, 0.13876683])
...
def load_and_preprocess_image(img_path: str):
    ...
    img_array = custom_preprocess_input(img_array)  # Χρήση των νέων mean και std τιμών
    return img_array
...
def calculate_distances_for_cases(base_dir: str, output_file: str):
    ...
    for case in sorted(os.listdir(base_dir)):
        ...
        if os.path.isdir(case_dir):
            ...
            print(f"Case {case}") #- Conv5_x Distance: {avg_distance_conv5:.4f}, Average Pooling Distance: {avg_distance_avgpool:.4f}, Fully Connected Layer Distance: {avg_distance_fc:.4f}")
...
if __name__ == "__main__":
    ...
    output_file = r"C:\Users\steli\DIPLOMA\myProgramms\XLSX\ResNet_Distances_CustomMeanStd.xlsx"
