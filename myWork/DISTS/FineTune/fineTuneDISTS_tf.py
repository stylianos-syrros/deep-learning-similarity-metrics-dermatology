...
# Set up logging
logging.basicConfig(filename="D:\\Diploma\\DISTS_FINE_TUNE\\training_log.txt", level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')
...
def fine_tune_dists_multiple_epochs(train_dir, validation_dir, doctor_scores, epochs=25):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            # Save epoch data
            current_alpha, current_beta = sess.run([model.alpha, model.beta])
            scio.savemat(f"D:\\Diploma\\DISTS_FINE_TUNE\\Mat_files\\epoch_{epoch+1}_alpha_beta.mat", {'alpha': current_alpha, 'beta': current_beta})
            ...
            if average_validation_loss < best_val_loss:
                ...
                # Save best model
                scio.savemat("D:\\Diploma\\DISTS_FINE_TUNE\\Mat_files\\best_model_alpha_beta.mat", {'alpha': best_alpha, 'beta': best_beta})
