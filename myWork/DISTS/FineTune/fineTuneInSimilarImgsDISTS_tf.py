...
# Set up logging
logging.basicConfig(filename="D:\\Diploma\\DISTS_FINE_TUNE\\training_similar_imgs_log.txt", level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')
...
# Load the Excel file
file_path = r"C:\Users\steli\DIPLOMA\bcc\SCARS-SCORES-SIMILAR-IMG-NORMALIZED.xlsx"
...
normalized_socres = ['Normalized_DISTS']
doctor_scores = scores_df[normalized_socres]
        ...
        for epoch in range(epochs):
            ...
            for case_folder in case_folders_train:
                ...
                if case_number in doctor_scores.index:
                    ...
                    loss_value = np.square(mean_dist - case_scores)
                    epoch_loss += loss_value

                    # Update the model weights
                    sess.run(optimizer, feed_dict={x: img0, y: imgs1, doctor_scores_placeholder: [case_scores]})
                    sess.run(optimizer, feed_dict={x: img0, y: imgs2, doctor_scores_placeholder: [case_scores]})

                    print(f"Case {case_number}: Mean DISTS: {mean_dist}, Loss: {loss_value}")
                    print(f"Normalized_DISTS: {case_scores}")


            # Save epoch data
            current_alpha, current_beta = sess.run([model.alpha, model.beta])
            scio.savemat(f"D:\\Diploma\\DISTS_FINE_TUNE\\Mat_files_similar_imgs\\epoch_{epoch+1}_alpha_beta.mat", {'alpha': current_alpha, 'beta': current_beta})
            ...
            if epoch_loss < best_loss:
                ...
                # Save best model
                scio.savemat("D:\\Diploma\\DISTS_FINE_TUNE\\Mat_files_similar_imgs\\best_model_alpha_beta.mat", {'alpha': best_alpha, 'beta': best_beta})
                logging.info("Best model updated")
            
        # Save best model
        scio.savemat("D:\\Diploma\\DISTS_FINE_TUNE\\Mat_files_similar_imgs\\best_model_alpha_beta.mat", {'alpha': best_alpha, 'beta': best_beta})
        print("Best model saved with Loss:", best_loss)
        logging.info(f"Final Best Loss: {best_loss}")

train_dir = r"C:\Users\steli\DIPLOMA\bcc"
fine_tune_dists_multiple_epochs(train_dir, doctor_scores)