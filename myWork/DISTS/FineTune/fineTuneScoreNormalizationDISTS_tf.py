...
# Set up logging
logging.basicConfig(filename="D:\\Diploma\\DISTS_FINE_TUNE\\training_log_normalization.txt", level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')
...
def fine_tune_dists_multiple_epochs(train_dir, validation_dir, doctor_scores, epochs=25):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            # List to store distances for normalization
            all_distances = []
            ...
            for case_folder in case_folders_train:
                ...
                if case_number in doctor_scores.index:
                    ...
                    dist1 = sess.run(distances, feed_dict={x: img0, y: imgs1})
                    dist2 = sess.run(distances, feed_dict={x: img0, y: imgs2})
                    all_distances.extend([dist1, dist2])

            # Find min and max DISTS for normalization
            min_dist = np.min(all_distances)
            max_dist = np.max(all_distances)
            print(f"Epoch {epoch+1}: min_dist = {min_dist}, max_dist = {max_dist}")
            logging.info(f"Epoch {epoch+1}: min_dist = {min_dist}, max_dist = {max_dist}")

            # Training with normalized distances
            for case_folder in case_folders_train:
                case_number = int(os.path.basename(case_folder).replace('CASE', ''))
                if case_number in doctor_scores.index:
                    imgs = process_case_folder(case_folder)
                    img0 = np.expand_dims(imgs[0], axis=0)
                    ...
                    # Normalize distances to [1, 7]
                    normalized_dist1 = 1 + 6 * (dist1 - min_dist) / (max_dist - min_dist)
                    normalized_dist2 = 1 + 6 * (dist2 - min_dist) / (max_dist - min_dist)
                    mean_dist = np.mean([normalized_dist1, normalized_dist2])

                    case_scores = doctor_scores.loc[case_number].values
                    mean_doctor_score = np.mean(case_scores)
                    loss_value = np.square(mean_dist - mean_doctor_score)
                    ...
                    print(f"Case {case_number}: Mean DISTS: {mean_dist}, Loss: {loss_value}")
            ...
            for case_folder in case_folders_val:
                ...
                if case_number in doctor_scores.index:
                    ...
                    case_scores = doctor_scores.loc[case_number].values
                    mean_doctor_score = np.mean(case_scores)
                    validation_loss = np.square(mean_dist - mean_doctor_score)
                    total_validation_loss += validation_loss

                    print(f"Case {case_number}: Mean DISTS: {mean_dist}, Loss: {validation_loss}")
                    print(f"Doctor Scores: {case_scores}")

            # Save epoch data
            current_alpha, current_beta = sess.run([model.alpha, model.beta])
            scio.savemat(f"D:\\Diploma\\DISTS_FINE_TUNE\\Mat_files_normalization\\epoch_{epoch+1}_alpha_beta.mat", {'alpha': current_alpha, 'beta': current_beta})
            ...
            if average_validation_loss < best_val_loss:
                ...
                # Save best model
                scio.savemat("D:\\Diploma\\DISTS_FINE_TUNE\\Mat_files_normalization\\best_model_alpha_beta.mat", {'alpha': best_alpha, 'beta': best_beta})
