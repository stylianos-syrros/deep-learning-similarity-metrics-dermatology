...
class DISTS():
    ...
    def get_score(self, img1, img2, session):
        ...
        # Καθορισμός των βαρών για κάθε επίπεδο
        weights1 = [0.16259184534886537, 0.3108558526442156, 0.3038336238220734, 0.16412432895316986, 0.058594349231675676, 0]  # Τα βάρη κανονικοποιημένα
        weights2 = [0.21585427272327534, 0.16563675663580021, 0.4992801282012849, 0.11922884243963962, 0, 0]  # Τα βάρη κανονικοποιημένα
        #weights1 = [0.15, 0.15, 0.2, 0.25, 0.25]

        cosine_sims = []
        for k in range(len(feats0)):
        #for k in range(1, len(feats0)):
            # Μετατροπή των χαρακτηριστικών σε numpy arrays
            feat0_np = session.run(feats0[k], feed_dict={img1: ref, img2: dist})
        ...
        weighted_mean_sim = np.sum(np.array(cosine_sims) * np.array(weights1))
...
if __name__ == '__main__':
    ...
    dist_image_path = r"C:\Users\steli\DIPLOMA\bcc\CASE004\2.jpg"
    model = DISTS()
